...
species.groupby(by=['conservation_status']).scientific_name.count()
...
category_counts = species.groupby(['category', 'is_protected'])    .scientific_name.count().reset_index()    .sort_values(by='category')    .rename(columns={'scientific_name': 'count'})
...
category_pivot = category_counts.pivot(columns='is_protected',index='category',values='count')
...
# Minimum Detectable Effect would be 33.333... since it's 1/3 of 15%
# So, sample size would be 510
sample_size = 510
...
bryce = sample_size / obs_by_park[obs_by_park.park_name == 'Bryce National Park'].observations.item()
print('Bryce would take around {:.0f} week(s)'.format(bryce))
yellowstone = sample_size / obs_by_park[obs_by_park.park_name == 'Yellowstone National Park'].observations.item()
print('Yellowstone would take around {:.0f} week(s)'.format(yellowstone))
